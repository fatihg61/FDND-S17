<script>
  ...
  function validateDonationAmount(donationAmount) {
    ...
    return amount >= 10; // Set your minimum donation amount here
  }
  ...
  const handleSubmit = () => {
    ...
    if (expiryDate === "") {
      ...
      errors.expiryDate = "Please enter a future expiry date.";
    }
    ...
    if (donationType === "") {
      errors.donationType = "Please choose your donationType.";
    }

    if (donationAmount === "") {
      errors.donationAmount = "Please fill in your donation amount.";
    } else if (!validateDonationAmount(donationAmount)) {
      errors.donationAmount = "Minimum donation amount is $10.";
    }
    ...
    if (Object.keys(errors).length === 0) {
      ...
      // Redirect to '/paymentgood'
      window.location.href = "/paymentgood";
    } else {
  ...
  function handleFastPayButtonClick() {
    window.location.href = "/paymentgood";
  }
...
<body>
  <div class="formfield">
    <form class="form">
      <div class="payment_other">
        <p>Fast Payment</p>
        <br />
        <button
          name="apple-pay"
          aria-label="Pay with Apple Pay" 
          on:click={handleFastPayButtonClick}
          type="button"
        ...
        >
          ...
          >
            ...
            <defs>
              <clipPath id="a">
                <path fill="#fff" d="M0 0h80v38.1H0z" />
              </clipPath>
            </defs>
          </svg>
        </button>
      ...
      <div class="card-info">
        <div class="input_container">
          <label class="input_label" for="cardHolderName">Cardholder Name</label
          >
          <input
            id="cardHolderName"
            class="input_field"
          ...
          />
          {#if errors.cardHolderName}<p class="error">
              {errors.cardHolderName}
            </p>{/if}
        </div>
        <div class="input_container">
          <label class="input_label" for="cardNumber">Card Number</label>
          <input
            id="cardNumber"
            class="input_field"
            type="text"
          ...
          />
          {#if errors.cardNumber}<p class="error">{errors.cardNumber}</p>{/if}
        </div>
        <div class="input_container">
          <label class="input_label" for="expiryDate">Expiry Date</label>
          <div class="split"></div>
          <input
            id="expiryDate"
            class="input_field"
          ...
          />
          {#if errors.expiryDate}<p class="error">{errors.expiryDate}</p>{/if}
        </div>
        <div class="input_container">
          ...
          <input
            ...
            bind:value={cvv}
          />
          {#if errors.cvv}<p class="error">{errors.cvv}</p>{/if}
        </div>
        <div class="input_container">
          <label class="input_label" for="donationType">Donation Type</label>
          <div class="split">
            <select
              id="donationType"
              class="input_field"
              style="width: 250px;"
              autocomplete="donation-type"
          ...
          {#if errors.donationType}<p class="error">
              {errors.donationType}
            </p>{/if}
        </div>

        <div class="input_container">
          <label class="input_label" for="donationAmount">Donation Amount</label
          >
          <input
            id="donationAmount"
            class="input_field"
          ...
          />
          {#if errors.donationAmount}<p class="error">
...
<style>
  ...
  .payment_other button svg {
    height: 18px;
  }
  .payment_other button:last-child svg {
    height: 22px;
  }

  .payment_other button:hover::before {
    transform: translate3d(100%, 0, 0);
  }
  .payment_other button:active {
